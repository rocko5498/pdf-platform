    ...
    fn recognize(
        ...
        match result {
            Ok(output) => {
                ...
                // Parse TSV output.
                read_and_parse_tsv(&tsv_path, page_index)
            }
            Err(e) => OcrPageResult {
                page_index,
                blocks: Vec::new(),
...
fn read_and_parse_tsv(tsv_path: &std::path::Path, page_index: u32) -> OcrPageResult {
    match std::fs::read_to_string(tsv_path) {
        Ok(tsv_content) => parse_tesseract_tsv(&tsv_content, page_index),
        Err(e) => OcrPageResult {
            page_index,
            blocks: Vec::new(),
            full_text: String::new(),
