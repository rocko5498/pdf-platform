...
pub fn hash_byte_ranges(file_bytes: &[u8], byte_range: &[u64]) -> Vec<u8> {
    ...
    while i + 1 < byte_range.len() {
        ...
        if offset + length <= file_bytes.len() {
            let _ = hasher.write_all(&file_bytes[offset..offset + length]);
        }
        i += 2;
    }
    hasher.finish()
...
// ---------------------------------------------------------------------------
// Minimal SHA-256 (for ByteRange hashing)
// ---------------------------------------------------------------------------
// A standalone SHA-256 implementation to avoid pulling in crypto dependencies
// for the validation framework. Full CMS verification uses ring/aws-lc in M10.
...
impl Sha256Writer {
    ...
    fn finish(mut self) -> Vec<u8> {
        ...
        let final_data = self.buffer.clone();
        for chunk in final_data.chunks(64) {
