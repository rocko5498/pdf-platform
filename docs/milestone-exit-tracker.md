...
| **M6** | Job scheduler core | **Core + restart persistence met; utility IPC pending** | Validated DAGs, bounded queue, ADR priority classes, cooperative cancellation, progress/terminal events, fixed named worker threads; actor-ordered append-only checkpoints recover past torn tails, normalize interrupted jobs to pending, and resume unfinished DAG nodes without rerunning completed prerequisites [ADR-009, ADR-021, SDS §2.2.3, SDS §4.6, FR-BATCH-4] |
