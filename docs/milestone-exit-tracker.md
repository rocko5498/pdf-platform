...
| **M6** | Job scheduler core | **Scheduler + utility transport/pool met; operation adapters pending** | Validated bounded DAG scheduler with restart persistence and retry-once for idempotent worker loss; bounded binary job/result frames cross the real IPC boundary to a fixed sandbox-spawned process pool. The registry currently implements `noop`; operation-specific adapters, brokered outputs, and OS low-priority policy remain [ADR-008, ADR-009, ADR-021, ADR-031, SDS §2.2.3, SDS §4.6, FR-BATCH-4] |
