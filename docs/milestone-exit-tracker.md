...
| **M6** | Job scheduler core | **Core + persistence format met; checkpoint/utility IPC pending** | Validated DAGs, bounded queue, ADR priority classes, cooperative cancellation, progress/terminal events, fixed named worker threads; versioned bounded append-only snapshots recover past torn tails and normalize interrupted jobs to pending [ADR-009, ADR-021, SDS §2.2.3] |
