...
| **M6** | Job scheduler core | **Core + restart persistence + crash retry met; utility IPC pending** | Validated DAGs, bounded queue, ADR priority classes, cooperative cancellation, progress/terminal events, fixed named worker threads; actor-ordered checkpoints restore unfinished DAG nodes; typed utility-worker loss retries explicitly idempotent jobs once; persisted format v2 retains v1 read compatibility [ADR-008, ADR-009, ADR-021, SDS §2.2.3, SDS §4.6, FR-BATCH-4] |
